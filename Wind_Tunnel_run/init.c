 ...
 *********************************************************************** */
{
  ...
  /* Safe placeholder values -- InitDomain() overwrites these with the
     real interpolated highmu dataset right after Startup() calls Init().
     Needed because PLUTO validates density/pressure immediately after
     Init(), before InitDomain() runs. */
  v[RHO] = g_inputParam[D_FLOOR];
  v[VX1] = 0.0;
  v[VX2] = 0.0;
  v[VX3] = 0.0;
  v[PRS] = g_inputParam[P_FLOOR];
  v[TRC] = 1.0;
}
 ...
 *********************************************************************** */
{
  ...
  int input_var[] = {RHO, VX1, VX2, VX3, PRS, TRC, -1};
  InputDataSet("INPUT_DATA/highmu/grid.out", input_var);
  InputDataRead("INPUT_DATA/highmu/data.1000.dbl", "little");

  TOT_LOOP(k, j, i)
  {
    InputDataInterpolate(vs, x1[i], x2[j], x3[k]);
    d->Vc[RHO][k][j][i] = vs[RHO];
    d->Vc[VX1][k][j][i] = vs[VX1];
    d->Vc[VX2][k][j][i] = vs[VX2];
    d->Vc[VX3][k][j][i] = vs[VX3];
    d->Vc[PRS][k][j][i] = vs[PRS];
    d->Vc[TRC][k][j][i] = vs[TRC];
...
/*----------------------------------------------------------------------------------------*/  
  ...
  if (side == X1_END)
  {
    X1_END_LOOP(k, j, i)
    {
      ...
      /* Validate cell values and enforce floors, same as X1_BEG */
      d->Vc[RHO][k][j][i] = MAX(d->Vc[RHO][k][j][i], g_inputParam[D_FLOOR]);
      d->Vc[PRS][k][j][i] = MAX(d->Vc[PRS][k][j][i], g_inputParam[P_FLOOR]);
