 ...
 *********************************************************************** */
{
...
#if COOLING == NO || COOLING == POWER_LAW
  ...
  if (v[TRC] < 1.005) // Using tracer = 10 for ambient medium and tracer = 1 for disk
  {
    mu = 2.0 * (CONST_AH + FRAC_He * CONST_AHe + FRAC_Z * CONST_AZ) /
         (2.0 + 3.0 * FRAC_He + FRAC_Z * (1.0 + CONST_AZ * 0.5));
  }
  else
  {
...
*/
  ...
  v[nv] /= mfsum;
  NIONS_LOOP(nv)
  numden[nv - NFLX] = v[RHO] * UNIT_DENSITY * (v[nv] / molmass[nv - NFLX]);
